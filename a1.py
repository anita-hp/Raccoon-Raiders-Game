"""CSC148 Assignment 1: Raccoon Raiders game
...
class GameBoard:
    ...
    _raccoon: List[Optional[Raccoon]]
    _bins: List[Optional[RecyclingBin, GarbageCan]]
    ...
    def place_character(self, c: Character) -> None:
        ...
        if isinstance(c, Player):
            self._player = c
        elif isinstance(c, (Raccoon, SmartRaccoon)):
            self._raccoon.append(c)
        elif isinstance(c, (RecyclingBin, GarbageCan)):
            self._bins.append(c)
    ...
    def at(self, x: int, y: int) -> List[Character]:
        ...
        True
        """
        if self._player is not None and self._player.x == x and \
                self._player.y == y:
            return [self._player]
        lst = []
    ...
    def to_grid(self) -> List[List[chr]]:
        ...
        if self._player is not None:
            grid[self._player.y][self._player.x] = self._player.get_char()
        for b in self._bins:
            grid[b.y][b.x] = b.get_char()
        for r in self._raccoon:
            grid[r.y][r.x] = r.get_char()
        return grid
    ...
    def check_game_end(self) -> Optional[int]:
        ...
        """
        for r in self._raccoon:
            if not r.inside_can and not r.check_trapped():
                self.ended = False
                return None
...
class RecyclingBin(Character):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        True
        """
        tile_x = self.x + direction[0]
        tile_y = self.y + direction[1]
        if not self.board.on_board(tile_x, tile_y):
            return False
        character_lst = self.board.at(tile_x, tile_y)
        if len(character_lst) == 0:
            self.x = tile_x
            self.y = tile_y
            return True
        character = character_lst[0]
        if character.get_char() != 'B':
            return False
        # tile occupied by Recycling Bin
...
class Player(TurnTaker):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        c = grid[tile_y][tile_x]
        if c == '-':
            self.y = tile_y
            self.x = tile_x
            return True
        character = self.board.at(tile_x, tile_y)[0]
        # tile occupied by Recycling Bin
        if c == 'B':
            if character.move(direction):
                self.y = tile_y
                ...
                return False
        # tile occupied by Open GarbageCan
        if c == 'O':
...
class Raccoon(TurnTaker):
    ...
    def check_trapped(self) -> bool:
        ...
        for direction in DIRECTIONS:
            tile_x = self.x + direction[0]
            tile_y = self.y + direction[1]
            if self.board.on_board(tile_x, tile_y):
                c = grid[tile_y][tile_x]
                if c in ('-', 'C', 'O'):
                    return False
        return True
    ...
    def take_turn(self) -> None:
        ...
        if not self.inside_can:
            ...
            for d in direction:
                tile_x = self.x + d[0]
                tile_y = self.y + d[1]
                if self.board.on_board(tile_x, tile_y) and self.move(d):
                    return None
        return None
...
class SmartRaccoon(Raccoon):
    ...
    def take_turn(self) -> None:
        ...
        """
        if not self.inside_can:
            min_dis = max(self.board.width, self.board.height)
            best_direction = (0, 0)
            for direction in DIRECTIONS:
                dis = self._helper_dir(direction)
                if dis != 0 and dis < min_dis:
                    min_dis = dis
                    best_direction = direction
            if best_direction == (0, 0):
                best_direction = get_shuffled_directions()[0]
            self.move(best_direction)

    def _helper_dir(self, direction: Tuple[int, int]) -> int:
        """
        Return the distance between the Smart Raccoon and the closest GarbageCan
        ...
        tile_y = self.y + direction[1]
        distance = 0
        while self.board.on_board(tile_x, tile_y):
            lst = self.board.at(tile_x, tile_y)
            if len(lst) == 0:
                c = '-'
            else:
                c = lst[0].get_char()
